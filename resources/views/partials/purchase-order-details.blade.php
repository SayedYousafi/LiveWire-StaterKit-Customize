      ...
      <tbody>
        ...
        <tr>
          <td colspan="8" class="text-center space-x-2 py-3">
            ...
            @php
            $missingInfo = (
            $itemOrder->is_po === "No" && empty($itemOrder->url)
            ) || empty($itemOrder->price_rmb);
            @endphp

            @if ($missingInfo)
            <flux:button size="sm" variant="default" wire:click="confirmPurchase({{ $itemOrder->item_id }})">
              Enter Missing Info
            </flux:button>
            @else
            <flux:button size="sm" icon="currency-dollar" class="bg-fuchsia-700! hover:bg-fuchsia-600! text-white!"
              wire:click="purchase('{{ $itemOrder->master_id }}', {{ $itemOrder->item_id }}, '{{ $itemOrder->item_name }}')"
