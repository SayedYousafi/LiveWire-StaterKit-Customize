    ...
    <flux:modal name="myModal" class="!w-[50rem] max-w-none">
        <div class="space-y-6">
            ...
            <div class="grid grid-cols-4 gap-4">
                ...
                <div class="col-span-2">
                    <flux:input wire:model="last_name" label="Last Name" placeholder="Last Name" class="w-full" :disabled="$isUpdate && !$enableEdit" />
                </div>
                <div class="col-span-2">
                    <flux:input wire:model="middle_name" label="Middle Name" placeholder="Middle Name" class="w-full" :disabled="$isUpdate && !$enableEdit" />
                </div>
                <div class="col-span-2">
                    <flux:input wire:model="email_private" label="Private Email" placeholder="Email" class="w-full" :disabled="$isUpdate && !$enableEdit" />
                </div>
                <div class="col-span-2">
                    <flux:input wire:model="email_business" label="Business Email" placeholder="Business Email" class="w-full" :disabled="$isUpdate && !$enableEdit" />
                </div>
                <div class="col-span-2">
                    <flux:input wire:model="mobile" label="Mobile" placeholder="Mobile Number" class="w-full" :disabled="$isUpdate && !$enableEdit" />
                </div>
                <div class="col-span-2">
                    <flux:input wire:model="phone" label="Phone" placeholder="Phone Number" class="w-full" :disabled="$isUpdate && !$enableEdit" />
                </div>

                <div class="col-span-2">
                    <flux:dropdown :disabled="$isUpdate && !$enableEdit">
                ...
                <div class="col-span-2">
                    <flux:dropdown :disabled="$isUpdate && !$enableEdit">
                        <flux:button icon:trailing="chevron-down">
                            {{ $marital_status ? ucwords(str_replace('_', ' ', $marital_status)) : 'Select Marital Status' }}
                        </flux:button>
                        <flux:menu>
                            <flux:menu.item wire:click="$set('marital_status', 'Single')">Single</flux:menu.item>
                            <flux:menu.item wire:click="$set('marital_status', 'in_relationship')">In Relationship</flux:menu.item>
                            <flux:menu.item wire:click="$set('marital_status', 'Married')">Married</flux:menu.item>
                            <flux:menu.item wire:click="$set('marital_status', 'Divorced')">Divorced</flux:menu.item>
                ...
                <div class="col-span-2">
                    <flux:input type='date' wire:model="dob" label="Date of Birth" placeholder="YYYY-MM-DD" class="w-full" :disabled="$isUpdate && !$enableEdit" />
                </div>
                <div class="col-span-2">
                    <flux:input wire:model="designation" label="Designation" placeholder="Designation" class="w-full" :disabled="$isUpdate && !$enableEdit" />
                </div>
                <div class="col-span-2">
                    <flux:input type='date' wire:model="join_date" label="Join Date" placeholder="YYYY-MM-DD" class="w-full" :disabled="$isUpdate && !$enableEdit" />
                </div>
                <div class="col-span-4">
                    <flux:input wire:model="note" label="Note" placeholder="Additional Notes" class="w-full" :disabled="$isUpdate && !$enableEdit" />
                </div>
            </div>

            <div class="flex">
                <flux:spacer />
                <flux:button type="button" variant="ghost" icon="x-circle" wire:click="cancel" x-on:click="Flux.modal('myModal').close()">
                    Cancel
                </flux:button>
                <flux:button type="submit" wire:click="{{ $isUpdate ? 'update' : 'save' }}" icon="plus-circle" variant="primary">
