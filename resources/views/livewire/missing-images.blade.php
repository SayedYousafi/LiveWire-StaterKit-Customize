    ...
    <div class="space-y-2">
        ...
        <div class="flex items-center justify-between bg-gray-100 p-2 rounded mb-2">
            ...
            <div class="flex items-center gap-3 flex-1">
                <img src="{{ asset('storage/' . $image) }}" alt="Image" class="w-16 h-16 object-cover rounded border" />
                <span>{{ $image }}</span>
            </div>
            <div class="flex gap-2 shrink-0">
                <flux:button as a icon='eye' variant='primary' href="{{ asset('storage/' . $image) }}" target="_blank"
                    class="bg-blue-500 text-white px-3 py-1 rounded hover:bg-blue-600">
                    View large
                </flux:button>
                <flux:button wire:click="deleteImage('{{ $image }}')" icon='minus-circle' variant='danger'>
                    Delete
                </flux:button>
            </div>
        </div>
